...
if __name__ == "__main__":
	...
	# CFC START
		...
		config = config_template.format(bw=bw, trace=trace, topo=topo, cc=args.cc, mode=2, t_alpha=1, t_dec=4, t_inc=300, g=0.00390625, ai=ai, hai=hai, dctcp_ai=1000, has_win=0, vwin=0, us=0, u_tgt=u_tgt, mi=mi, int_multi=1, pint_log_base=pint_log_base, pint_prob=pint_prob, ack_prio=1, link_down=args.down, failure=failure, kmax_map=kmax_map, kmin_map=kmin_map, pmax_map=pmax_map, buffer_size=bfsz, enable_tr=enable_tr)
	# CFC END
	elif args.cc == "hp":
		ai = 10 * bw / 25;
		if args.hpai > 0:
