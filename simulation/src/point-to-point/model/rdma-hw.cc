...
void RdmaHw::HandleAckHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	...
	/*
	if (Simulator::Now().GetNanoSeconds() > qp->hp.m_lastUpdateTime){ // if full RTT feedback is ready, do full update
	*/
...
void RdmaHw::UpdateRateHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react){
	...
	// backward notification
	/*
	if (qp->hp.m_lastUpdateTime == 0){ // first RTT
		qp->hp.m_lastUpdateTime = Simulator::Now().GetNanoSeconds() + qp->m_baseRtt; // legend says that qp->m_baseRtt was actually multiplied by 0.75
	*/
		// store INT
		...
		if (!fast_react){
			...
			// backward notification
			/*
			if (Simulator::Now().GetNanoSeconds() > qp->hp.m_lastUpdateTime)
				qp->hp.m_lastUpdateTime = Simulator::Now().GetNanoSeconds() + qp->m_baseRtt; // legend says that qp->m_baseRtt was actually multiplied by 0.75
			*/			
		}
	}
